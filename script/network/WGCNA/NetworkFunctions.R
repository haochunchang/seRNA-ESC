...
SoftConnectivity=function(datE, power=6,batchsize=1500,MinimumNoSamples=10) {
  ...
  if (no.genes<10 | no.samples<5 ) {
    print("Error: Something seems to be wrong. Make sure that the input data frame has genes as rows and array samples as columns. Alternatively, there could be fewer than 10 genes or fewer than 5 samples. ") } 
  else {
    if (no.genes<no.samples ) {
      print("Warning: There are fewer genes than samples in the function SoftConnectivity. Maybe you should transpose the data?") }
...
TOMdist1=function(adjmat1, maxADJ=FALSE) {
  ...
  if (maxh1>1 | minh1 < 0 ) {
    ...
    if (  max(c(as.dist(abs(adjmat1-t(adjmat1)))))>10^(-12)   ) {
